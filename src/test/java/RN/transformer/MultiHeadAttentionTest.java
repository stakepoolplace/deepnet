...
public class MultiHeadAttentionTest {
    ...
    public void testBackward() {
        ...
        // Entrées fictives
        INDArray query = Nd4j.rand(seqLength, dModel);
        INDArray key = Nd4j.rand(seqLength, dModel);
        INDArray value = Nd4j.rand(seqLength, dModel);

        // Masque fictif (optionnel)
        INDArray mask = null; // Ou créez un masque de forme [numHeads, seqLength, seqLength] si nécessaire

        // Passe forward
        INDArray output = mha.forward(query, key, value, mask);

        // Vérification des dimensions de la sortie
        assertEquals(seqLength, output.rows(), "Le nombre de lignes de la sortie devrait correspondre à seqLength");
        assertEquals(dModel, output.columns(), "Le nombre de colonnes de la sortie devrait correspondre à dModel");

        // Création d'un gradOutput fictif
        INDArray gradOutput = Nd4j.rand(seqLength, dModel);
